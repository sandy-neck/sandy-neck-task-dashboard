...
| `reply-ingest-state.json` | Just the last IMAP UID `agent/reply_ingest.py` has already pulled in — not sales/business data, but lives here for the same reason: small state that shouldn't be re-derived or re-fetched each run. Replies to the daily email land in `INBOX.md`'s Unprocessed section, not in this file. | `agent/reply_ingest.py`, one update per run (only when IMAP credentials are configured). |
